...
//Code that was taken from other files are commented
...
void create_dirent(struct direntry *dirent, char *filename, 
		   uint16_t start_cluster, uint32_t size,
		   uint8_t *image_buf, struct bpb33* bpb)//also from doscp
{
    while (1) 
    {
...
int need(long file_size){
	return((511+file_size)/512);
}
...
uint16_t print_dirent(struct direntry *dirent, int indent,uint8_t *image_buf, struct bpb33 *bpb, int *clusts) //Mostly code from dos_ls
{
    ...
    int chain = 0;
	uint16_t target = 0;
    uint16_t clust = getushort(dirent->deStartCluster);
    while (is_valid_cluster(clust, bpb))
    {
    	clusts[clust] ++;
        uint16_t prev = clust;
        clust = get_fat_entry(clust, image_buf, bpb);
        if(need(size)==1 && chain==0){
            target = prev;
        }//endif
        if(clust == prev)
        { 
            printf("\n Looped Cluster\n");
            set_fat_entry(clust, FAT12_MASK& CLUST_EOFS, image_buf, bpb);
            chain++;
            break;
        }//endif
    	if(clust==(FAT12_MASK&CLUST_BAD)) {
        	target = clust;
        	set_fat_entry(clust, FAT12_MASK & CLUST_FREE, image_buf, bpb);
        ...
        chain++; 
        if((need(size)==chain+1)){
	 		target = clust;
		}//endif
	} //endwhile

    if(need(size) > chain){
        printf("Metadata Inconsistencies\n");
        printf("Size: %d Clusters\n Chain Length: %d Clusters\n", need(size), chain);
		int change = chain*512;
        putulong(dirent->deFileSize, change);
    }//endif

	if(need(size) < chain){	
       printf("\n Number of Clusters is too long\n");
       printf("\nSize: %d Clusters\n Chain Length: %d Clusters \n", need(size), chain);
       clust = get_fat_entry(target,image_buf,bpb);
       set_fat_entry(target, FAT12_MASK&CLUST_EOFS, image_buf, bpb);
       free_cluster(image_buf, bpb, clust, clusts);
    }//endif
	} 
...
void search(uint8_t *image_buf, struct bpb33 *bpb, int *clusts){
	...
	for (i = 0; i < 7; i++)
   	...
   	orphan_size =(tmp->orphansize)*512;
  	create_dirent(dirent,filename, tmp->clustnum, orphan_size, image_buf, bpb);
        tmp= tmp->next;
        numorph++;
     }
     list_clear(orphans);	
...
void follow_dir(uint16_t cluster, int indent, uint8_t *image_buf, struct bpb33* bpb,int *clusts){ //from dos_ls just added clusts
    while (is_valid_cluster(cluster, bpb)){
...
void traverse_root(uint8_t *image_buf, struct bpb33* bpb, int *clusts) //from dos_ls just added clusts
{
    uint16_t cluster = 0;
...
void usage(char *progname) { //from dos_ls
...
int main(int argc, char** argv) {
    ...
    u_int16_t sectors = (bpb->bpbSectors) - ((bpb->bpbFATsecs)*2)- 15;
